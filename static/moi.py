...
def ana_sayfayi_Guncelle(proje_listesi):
    ...
    for proje_bilgisi in sirali_proje_listesi: # Sıralı proje listesini döngüye al
        ...
        for dosya_adi in os.listdir(proje_klasor_yolu): # Proje klasöründeki dosyaları listele (YENİ DÖNGÜ)
            if dosya_adi.lower().startswith("thumbnail.") and dosya_adi.lower().endswith((".jpg", ".jpeg", ".gif", ".png")): # Eğer dosya adı "thumbnail." ile başlıyor VE uzantısı .jpg, .jpeg, .gif veya .png ise (YENİ KOŞUL)
                thumbnail_dosya_adi = dosya_adi # Thumbnail dosya adını kaydet (YENİ)
                thumbnail_dosya_yolu = os.path.join(proje_klasor_yolu, thumbnail_dosya_adi) # Thumbnail dosya yolunu oluştur (YENİ)
                break # İlk bulunan thumbnail'i kullan ve döngüden çık (YENİ)
        ...
        if thumbnail_dosya_yolu and os.path.exists(thumbnail_dosya_yolu): # Thumbnail dosyası bulundu ve gerçekten var mı kontrolü (GÜNCEL KOŞUL)
            ...
            proje_listesi_html_satiri = f"""<div class="brick"><a href="{proje_linki}"><img src="{thumbnail_url}" alt="{proje_basligi}"></a></div>""" # Proje listesi HTML satırını thumbnail ve proje başlığı ile oluştur - GÜNCELLENDİ
